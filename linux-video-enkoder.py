...
class VideoConverterWindow(Gtk.Window):
    def __init__(self):
        ...
        grid.attach(Gtk.Label(label="Analyse-Stufe:", xalign=0), 0, 5, 1, 1)
        self.preset_combo = Gtk.ComboBoxText()
        presets = ["ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"]
        for p in presets: self.preset_combo.append_text(p)
        ...
        # Reihe 1: Konvertieren und Abbrechen
        ...
        # Reihe 2: Beenden (links) und Zurücksetzen (rechts)
        self.exit_btn = Gtk.Button(label="Schließen")
        ...
        self.file_label = Gtk.Label(label="Dateifortschritt:", xalign=0)
    ...
    def on_reset_all(self, btn):
        ...
        for combo in [self.gpu_combo, self.upscale_combo, self.audio_combo,
            ...
            combo.set_active(0)
        self.preset_combo.set_active(5)
        self.append_log("Programm zurückgesetzt.\n")

    def on_open_preview(self, btn):
        selection = self.treeview.get_selection()
        model, paths = selection.get_selected_rows()
        video_path = self.selected_files[paths[0].get_indices()[0]] if paths else (self.selected_files[0] if self.selected_files else None)
        if video_path and VideoPreviewDialog:
    ...
    def on_quality_mode_changed(self, combo):
        ...
        if "CQ" in mode: self.quality_label.set_text("CRF Wert (0-51):"); self.quality_entry.set_text("23")
        elif "Bitrate" in mode: self.quality_label.set_text("Bitrate (kbit/s):"); self.quality_entry.set_text("5000")
        else: self.quality_label.set_text("Zieldateigröße (MB):"); self.quality_entry.set_text("700")
    ...
    def on_drag_data_received(self, widget, context, x, y, selection, info, time):
        for uri in selection.get_uris():
            ...
            if Path(path).exists() and path not in self.selected_files:
                self.selected_files.append(path); self.liststore.append([Path(path).name])
    ...
    def build_ffmpeg_args(self, infile, outfile):
        ...
        qmode, qval = self.quality_combo.get_active_text(), self.quality_entry.get_text().strip()
        ...
        args += ["-c:a", {"AAC":"aac","PCM":"pcm_s16le","FLAC (mkv)":"flac"}.get(achoice, "copy")]
    ...
    def start_conversion(self, btn):
        ...
        self.start_btn.set_sensitive(False); self.cancel_btn.set_sensitive(True); self.reset_btn.set_sensitive(False)
    ...
    def run_conversion(self):
        ...
        for idx, infile in enumerate(list(self.selected_files), 1):
            ...
            elif self.target_entry.get_text(): out_dir = Path(self.target_entry.get_text())
            else: out_dir = in_p.parent / "converted"
            ...
            self.append_log(f"\n--- Datei {idx}/{total}: {in_p.name} ---\n")
            try:
                self.current_proc = subprocess.Popen(["ffmpeg"] + args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
                for line in self.current_proc.stdout:
                    self.append_log(line)
                    m = time_re.search(line)
                    if m:
                        secs = int(m.group(1))*3600 + int(m.group(2))*60 + float(m.group(3))
                        pct = min(1.0, secs / dur)
