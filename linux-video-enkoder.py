...
# Version:  1.0.8 (Audio Copy Feature)
...
def calculate_bitrate_for_target_size(filepath, target_size_mb, audio_bitrate_kbps=192):
    ...
    # Wenn Audio kopiert wird, ist die Bitrate schwerer vorhersagbar,
    # wir nehmen hier einen Standardwert an.
...
class VideoConverterWindow(Gtk.Window):
    def __init__(self):
        ...
        # NEU: Audio kopieren Checkbutton
        self.audio_copy_chk = Gtk.CheckButton(label="Audio kopieren (Stream Copy)")
        self.audio_copy_chk.connect("toggled", self.on_audio_copy_toggled)
        grid.attach(self.audio_copy_chk, 1, 2, 1, 1)

        grid.attach(Gtk.Label(label="Videoformat:", xalign=0), 0, 3, 1, 1)
        ...
        grid.attach(self.video_combo, 1, 3, 1, 1)

        grid.attach(Gtk.Label(label="Qualität Modus:", xalign=0), 0, 4, 1, 1)
        ...
        grid.attach(self.quality_combo, 1, 4, 1, 1)

        self.quality_label = Gtk.Label(label="CRF Wert (0-51):", xalign=0)
        grid.attach(self.quality_label, 0, 5, 1, 1)
        self.quality_entry = Gtk.Entry(text="23")
        grid.attach(self.quality_entry, 1, 5, 1, 1)

        grid.attach(Gtk.Label(label="Analyse-Stufe:", xalign=0), 0, 6, 1, 1)
        ...
        grid.attach(self.preset_combo, 1, 6, 1, 1)
    ...
    def on_audio_copy_toggled(self, btn):
        # Audio-Auswahl deaktivieren, wenn "Kopieren" gewählt ist
        self.audio_combo.set_sensitive(not btn.get_active())
    ...
    def run_conversion(self):
        ...
        for idx, infile in enumerate(list(self.selected_files), 1):
            ...
            # Falls Audio kopiert wird, behalten wir oft die Endung des Originals bei,
            # es sei denn es ist MKV gewünscht.
            if audio_copy:
